...
from configuration.config import logger, app_config
from source.exceptions.commons import NotOkServiceResponse
from source.exceptions.custom_exceptions import MetadataObjParseError, FileTypeExtractionError
...
def parse_metadata_id_object(metadata_object: list[dict]) -> str:
    ...
    try:
        ...
        raise MetadataObjParseError(detail=f"Could not parse the metadata object")
    if not file_name:
        raise MetadataObjParseError(detail=f"Could not fetch the file_name")
    return file_name


def guess_and_extract_content_type(file_name: str) -> (str, str):
    """Based on the file's name guess the media type in the format of application/* and return both formats"""
    mapping_extension_with_mimetypes = {
    ...
    try:
        ...
        logging.error(error)
        raise FileTypeExtractionError(detail="Could not extract the filetype from the guesses mime type")

    try:
        detected_mime_type = mapping_extension_with_mimetypes[file_extension]
    except KeyError as error:
        logging.error(error)
        raise FileTypeExtractionError(detail=f"Could not fine mimetype for extension {file_extension}")

    return detected_mime_type, detected_mime_type.split('/')[1]


async def make_request_streaming(
        request: Request,
        service_url: str,
        uri: str,
        query_params: dict = None,
    ...
    async with ClientSession() as session:
        async with session.get(f'{service_url}{uri}', stream=True, headers=headers, params=query_params) as response:
            ...
            # You can process the response content in chunks
            async for chunk in response.content.iter_chunked(app_config.RESPONSE_CHUNK_SIZE):
