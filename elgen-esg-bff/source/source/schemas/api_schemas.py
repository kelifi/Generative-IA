...
from source.schemas.source_documents_schemas import WebSourceSchema, SourceSchema
...
class QuestionSchema(QuestionInput):
    ...
    local_sources: list[SourceSchema] | None = Field(description="the local document sources", default=[])
    web_sources: list[WebSourceSchema] | None = Field(description="the external web sources", default=[])
...
class QuestionLimitOutputSchema(QuestionSchema):
    questions_remaining: int = Field(description="number of remaining questions")
