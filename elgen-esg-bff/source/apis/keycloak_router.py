...
from configuration.injection import InjectionContainer, logger
...
from source.schemas.keycloak_schemas import LoginData, UserInfo, UserInfoRegistration, LoginModel, ClientRole, \
    UserCreationBulkResponse, UserEncrypted, PlatformUsers
...
@keycloak_router.get("/users",
                     summary="gets the users list of those registered into the platform",
                     response_model=PlatformUsers)
@inject
async def get_all_users(
    ...
    try:
        payload = await keycloak_service.check_auth(credentials)
        keycloak_service.check_user_role(payload, [ClientRole.SUPER_ADMIN])
        users = await keycloak_service.get_all_users()
        logger.info(f"{users}: logging users list")
        return PlatformUsers(users=users)
    except ValidationError as error:
        logging.error(error)
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                            detail="A schema validation error occurred!")
    except KeycloakError as keycloak_error:
        raise UnauthenticatedKeycloakAPIError(
            detail=keycloak_error.detail,
            status_code=keycloak_error.status_code
