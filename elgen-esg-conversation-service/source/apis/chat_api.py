...
from fastapi import APIRouter, Depends, Header, Path, Query, Body, HTTPException, status
...
from source.exceptions.service_exceptions import (ChatServiceError, ChatAnswerCreationError,
                                                  ChatIncompleteDataError, AnswerNotFoundError,
                                                  VersionedAnswerNotFoundException, ChatModelDiscoveryError,
                                                  SqlSourceResponseSavingException, SQLModelDiscoveryError,
                                                  ConversationFetchDataError, SQLExecuteError
                                                  )
...
@chat_router.get(path="/answer/{question_id}/sql/stream")
@inject
async def perform_sql_chain_by_streaming(request: Request,
                                         question_id: UUID = Path(...),
                                         user_id: UUID = Header(..., alias='user-id'),
                                         workspace_id: UUID = Header(None, alias='workspace-id'),
                                         model_code: str = Header(..., alias='model-code'),
                                         query_depth: QueryDepth = Query(..., alias='queryDepth'),
                                         full_sql_chain_service: FullSQLChain = Depends(
                                             Provide[DependencyContainer.full_sql_chain_service])):
    """
    The true endpoint for streaming,
    """

    return StreamingResponse(
        full_sql_chain_service.generate_answer(request=request,
                                               question_id=question_id,
                                               chat_model_code=model_code,
...
@chat_router.get(path="/answer/{question_id}/sql")
@inject
async def generate_sql_query_with_llm(question_id: UUID = Path(...),
                                      user_id: UUID = Header(..., alias='user-id'),
                                      workspace_id: UUID = Header(None, alias='workspace-id'),
                                      model_code: str = Header(None, alias='model-code'),
                                      execute_sql_query: bool = False,
                                      service: SQLQueryChain = Depends(
                                          Provide[DependencyContainer.sql_llm_service])
                                      ) -> SqlSourceResponseDTO:
    """
    Generate an sql query from a human readable user question.
    """
    try:
        return await service.generate_answer(question_id=question_id, model_code=model_code, workspace_id=workspace_id)
    except (ConversationFetchDataError, SqlSourceResponseSavingException, SQLExecuteError) as error:
        logger.error(error)
        raise ElgenAPIException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Cannot generate sql query!",
        ) from error
    except SQLModelDiscoveryError as error:
        logger.error(error)
        raise ElgenAPIException(
            status_code=status.HTTP_503_SERVICE_UNAVAILABLE,
            detail="Unable to connect to the SQL LLM model!",
        ) from error
