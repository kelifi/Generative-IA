...
@model_router.get(path="/workspaces",
                  description="Get the available models allowed in the workspace assigned to the user ",
                  response_model=AvailableModelsOutputSchema,
                  summary="Get the list of available models for chat by workspace_id")
@inject
def get_available_models_for_chat_by_workspace_id(
        model_service: ModelService = Depends(Provide[DependencyContainer.model_service]),
        workspace_id: str = Query(..., description="the workspace type")):
    try:
        return model_service.get_models_for_chat_by_workspace_id(workspace_id=workspace_id)
...
# TODO make this endpoint more generic
@model_router.get(path="/types",
                  description="Get the models from the ELGEN database corresponding to the workspace type",
                  response_model=list[ModelInputSchema], summary="Get models corresponding to workspace type")
@inject
def get_models_per_workspace_type(model_service: ModelService = Depends(Provide[DependencyContainer.model_service]),
                                  workspace_type: str = Query(..., description="the workspace type")):
    try:
        return model_service.get_models_per_workspace_type(workspace_type=workspace_type)
    except (GenericValidationError, ModelRetrievalError) as error:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=error.message)


@model_router.get(path="/{model_code}",
                  description="Get the model's source config from the ELGEN database per model code",
